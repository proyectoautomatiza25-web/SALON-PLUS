            ...
            <div style={styles.gridContainer}>
                {currentView === 'list' ? (
                    <div style={{ padding: '20px', overflowY: 'auto', background: '#fff', height: '100%' }}>
                                ...
                                {(appointments || [])
                                    ...
                                    .map(app => (
                                        <tr key={app.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                            ...
                                            <td style={{ padding: '12px', fontSize: '0.9rem' }}>{(patients || []).find(p => String(p.id) === String(app.patientId))?.name || 'NP: ' + app.title}</td>
                        ...
                        {(!appointments || (appointments || []).filter(a => formatIsoDate(a.start) === formatIsoDate(currentDate) && String(a.profId) === String(selectedResourceId)).length === 0) && (
