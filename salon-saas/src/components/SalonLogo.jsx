...
const SalonLogo = ({ className = "h-10" }) => {
    const { businessName, businessLogo } = useSalonStore();

    return (
        <div className={`flex items-center gap-3 ${className}`}>
            {/* Icono Gráfico o Logo Subido */}
            {businessLogo ? (
                <img src={businessLogo} alt={businessName} className="h-[42px] w-auto max-w-[100px] object-contain rounded-md animate-in fade-in" />
            ) : (
            ...
            <div className="flex flex-col justify-center">
                <h1 className="text-2xl font-serif font-black tracking-tighter text-slate-800 leading-none truncate max-w-[200px]">
